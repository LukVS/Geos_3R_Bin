...
ind=min(find(phData.cycle_d==16369003));
v_ind=find(phData.cycle_d==16369003);
t = phData.time(find(~isnan(phData.phase)):end)';
r = phData.range(find(~isnan(phData.phase)):end)';
v = phData.Doppler(find(~isnan(phData.phase)):end)';
p = phData.phase(find(~isnan(phData.phase)):end)'/(F0_gln + dF_gln*10)*c;%  * (1 + drift*5*150/4.092e6*2 );
...
tmp=diff(p)./diff(t);
diff_p=tmp(ind_p);
dp=polyval( polyfit(t(ind_p), diff_p, 2), t(ind_p));
tmp=diff(r)./diff(t);
diff_r=tmp(ind_p);
dr=polyval( polyfit(t(ind_p), diff_r, 2), t(ind_p));
V_apr=polyval( polyfit(t, -v-drift, 2), t);
...
plot(phData.time(v_ind), (phData.ADR(v_ind)-phData.ADR(ind))*F0_gps/c*2, 'k');
grid on
